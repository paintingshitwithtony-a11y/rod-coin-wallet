...
export default function WalletImport({ account, onWalletImported }) {
    const [importMode, setImportMode] = useState('address');
    ...
    const handleImportFromSeed = async () => {
        ...
        setImporting(true);
        try {
            // Call backend to derive address from seed
            const response = await base44.functions.invoke('deriveFromSeed', {
            ...
            const derivedAddress = response.data.address;
            const derivedPrivateKey = response.data.private_key;
            ...
            if (currentAccount.wallet_address === derivedAddress) {
                toast.error('This seed phrase generates your primary address');
                setImporting(false);
                return;
            }

            if (additionalAddresses.some(addr => addr.address === derivedAddress)) {
                toast.error('This address is already added');
                setImporting(false);
                return;
            }

            // Add new address with seed phrase
            const newAddress = {
            ...
            toast.error('Failed to import wallet from seed phrase');
            console.error(error);
        } finally {
            setImporting(false);
        }
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <CardDescription className="text-slate-400">
                                Add an existing ROD wallet by address or seed phrase
                            </CardDescription>
                        </div>
                    </div>
                </CardHeader>
                <CardContent className="space-y-5">
                    <Tabs value={importMode} onValueChange={setImportMode}>
                        <TabsList className="grid w-full grid-cols-2 bg-slate-800">
                            ...
                            <TabsTrigger value="seed">By Seed Phrase</TabsTrigger>
                        </TabsList>
                        
                        <TabsContent value="address" className="space-y-5 mt-5">
                    ...
                    <div className="space-y-2">
                        ...
                        <TabsContent value="seed" className="space-y-5 mt-5">
                            <Alert className="bg-purple-500/10 border-purple-500/30">
                                <AlertCircle className="h-4 w-4 text-purple-400" />
                                <AlertDescription className="text-purple-300/80 text-sm">
                                    Enter your 12 or 24 word seed phrase to restore your wallet. The address and private key will be automatically derived.
                                </AlertDescription>
                            </Alert>
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea
                                    ...
                                    className="bg-slate-800/50 border-slate-700 text-white font-mono min-h-[100px]"
                                />
                                <p className="text-xs text-slate-500">
                            ...
                            <Alert className="bg-amber-500/10 border-amber-500/30">
                                <AlertCircle className="h-4 w-4 text-amber-400" />
                                <AlertDescription className="text-amber-300/80 text-sm">
                                    Never share your seed phrase with anyone. Your seed phrase is encrypted and stored securely.
                                </AlertDescription>
                            </Alert>
                        </TabsContent>
                    </Tabs>
                    ...
                    <Button
                        ...
                        disabled={importing || (importMode === 'address' ? (!addressValid || !label.trim()) : (!seedPhrase.trim() || !label.trim()))}
