        ...
        <>
            {displayUnreadCount > 0 && activeTab !== 'messages' && (
                ...
                >
                  ...
                  <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 border-t border-purple-500/30 bg-slate-950/95 backdrop-blur-xl px-2 pt-1 pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
                <div className="grid grid-cols-5 gap-1">
                    {tabs.map(({ value, label, Icon }) => (
                        <button
                            ...
                            className={`relative flex flex-col items-center justify-center gap-0.5 rounded-xl py-1.5 text-[9.5px] touch-manipulation select-none active:scale-95 transition-all ${activeTab === value ? 'text-amber-300 bg-purple-500/30' : 'text-slate-400'}`}
                        >
                            <span className="relative">
                                <Icon className="w-5 h-5" />
                                {value === 'messages' && <Badge />}
                            </span>
                            <span className="font-medium tracking-tight truncate max-w-[52px]">{label}</span>
                        </button>
                    ))}
                </div>
            </div>
