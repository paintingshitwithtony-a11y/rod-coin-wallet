...
export default function WalletDashboard({ account, onLogout }) {
  ...
  // NEW: Import Stored Private Keys (for your 10M ROD wallet)
  const handleImportStoredKeys = async () => {
    if (!rpcConnected) {
      toast.error("RPC connection required to import spendable wallets");
      return;
    }

    try {
      setLoading(true);
      toast.info("Scanning for stored private keys...");

      const result = await base44.functions.invoke('importStoredWallets', {
        accountId: account.id,
        labelPrefix: "Imported High Value"
      });

      if (result.data?.imported > 0) {
        toast.success(`✅ Imported ${result.data.imported} wallet(s)!`, {
          description: "Check My Addresses tab for the 10M ROD wallet"
        });
        await fetchAllWallets();
        await fetchWalletData();
      } else {
        toast.info("No new spendable keys found. Try manual import in Import tab.");
      }
    } catch (err) {
  ...
  // Paste the rest of your original functions here (fetchAllWallets, checkRPCStatus, etc.)
  ...
  return (
    <div className="space-y-4 md:space-y-6 overflow-x-hidden touch-pan-y" 
      ...
      {/* Quick Action Buttons - New Import Button Added Here */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <Card className="bg-slate-900/80 border-slate-700/50">
          <CardContent className="p-4 space-y-3">
            <div className="flex gap-2 flex-wrap">
              <Button onClick={() => handleTabChange('send')} className="flex-1">
                <ArrowUpRight className="w-4 h-4 mr-2" /> Send
              </Button>
              <Button onClick={() => handleTabChange('receive')} className="flex-1 bg-amber-500/20 text-amber-400">
              ...
              <Button
                variant="outline"
                onClick={handleImportStoredKeys}
                disabled={loading || !rpcConnected}
                className="flex-1 border-purple-500/50 text-purple-400 hover:bg-purple-500/10">
                <Unlock className="w-4 h-4 mr-2" />
                Import Stored Key (10M)
              </Button>
            </div>
          </CardContent>
        </Card>
      ...
      {/* Rest of your original dashboard content goes here */}
