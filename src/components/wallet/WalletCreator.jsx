...
import {
    ...
    Eye, EyeOff, ShieldAlert, KeyRound
} from 'lucide-react';
import { base44 } from '@/api/base44Client';
import { toast } from 'sonner';

/**
 * WalletCreator — 3-step wallet creation:
 *
 *  Step 1: "create"   — Enter name, passphrase, color
 *  Step 2: "recovery" — Show address + WIF private key, user must confirm saved
...
export default function WalletCreator({ account, onClose, onCreated }) {
    ...
    const [recoveryData, setRecoveryData] = useState(null); // { address, wif, walletId, walletName }
    ...
    const handleCreate = async () => {
        ...
        try {
            // ── Step A: Validate passphrase first (no address created yet) ──
            ...
            // ── Step B: Passphrase valid — now create the wallet ──
                        ...
                        <div className="space-y-4">
                            <Alert className="border-amber-500/50 bg-amber-500/10">
                                ...
                                <AlertDescription className="text-amber-300 text-sm">
                                    <strong>Save these details NOW.</strong> Your private key will never be shown again. If you lose it and your node is reset, you will permanently lose access to this wallet.
                                </AlertDescription>
                            </Alert>

                            <CopyField label="Wallet Address" value={recoveryData.address} mono />
                            ...
                            {recoveryData.wif ? (
                                ...
                                <Alert className="border-slate-600 bg-slate-800">
                                    ...
                                    <AlertDescription className="text-slate-400 text-sm">
                                        Private key could not be exported (wallet may have re-locked). Export it manually via RPC Console: <code className="text-amber-400">dumpprivkey {recoveryData.address}</code>
                                    </AlertDescription>
                                </Alert>
                            )}
                            ...
                            <div className="space-y-2 pt-1">
                                <label className="flex items-start gap-3 cursor-pointer group">
                                    ...
                                    <span className="text-sm text-slate-300 group-hover:text-white">
                                        I have saved the wallet address in a secure location
                                    </span>
                                </label>
                                <label className="flex items-start gap-3 cursor-pointer group">
                                    <input
                                    ...
                                    />
                                    <span className="text-sm text-slate-300 group-hover:text-white">
                                        I have saved the private key (or noted to export it) — I understand it will not be shown again
                                    </span>
                                </label>
                            </div>
